...
def save_snap(snap, dir):
    ...
    finalFileName = snap.sender + "_" + snap.snap_id + "." + fn.split(".")[2]
    finalFilePath = os.path.join(dir, finalFileName)
    os.rename(os.path.join(dir, fn), finalFilePath)
    if is_video_file(finalFilePath):
        scriptPath = os.path.join(os.getcwd(), "scripts", "extract_media_then_rename")
        subprocess.Popen([scriptPath, finalFilePath]).wait()
